...
select r_id, avg(price) as "AVG_Price" from menu group by r_id;

#3 Find the top restaurant in terms of the number of orders for a given month

SELECT re.r_name, COUNT(o.order_id) AS "total_orders"
FROM restaurants re
JOIN orders o ON re.r_id = o.r_id
...
SELECT u.user_id, u.name, o.order_id, fo.f_name, o.date 
FROM users u 
JOIN orders o ON u.user_id = o.user_id
JOIN order_details od ON o.order_id = od.order_id
JOIN food fo ON od.f_id = fo.f_id
WHERE u.user_id = 3 AND o.date BETWEEN '2022-05-10' AND '2022-06-29';

#6 Find restaurants with max repeated customers
WITH CustomerOrders AS (
    SELECT o.r_id, o.user_id, COUNT(*) AS order_count
    FROM orders o
...
RepeatedCustomerCount AS (
    SELECT r_id, COUNT(*) AS repeated_customer_count
    FROM RepeatedCustomers
    GROUP BY r_id
),
MaxRepeatedCustomers AS (
    SELECT MAX(repeated_customer_count) AS max_repeated_customer_count
    FROM RepeatedCustomerCount
)
SELECT r.r_name, r.cuisine, rc.repeated_customer_count
FROM RepeatedCustomerCount rc
JOIN MaxRepeatedCustomers mrc ON rc.repeated_customer_count = mrc.max_repeated_customer_count
...
from orders group by Month_No
),
Growth as (
select Month_No, Revenue, lag(Revenue) over (order by Month_No) as P_Month_Revenue from Month_Revenue
)
select Month_No, Revenue, P_Month_Revenue, (Revenue-P_Month_Revenue)/P_Month_Revenue*100 as GROWTH_BY_MONTH 
from Growth Where P_Month_Revenue is not Null;

#8 Customer — favourite food
WITH FoodCount AS (
     SELECT o.user_id, u.name, od.f_id, f.f_name, COUNT(*) AS order_count
     FROM orders o
     JOIN order_details od ON o.order_id = od.order_id
 ...
 RankedFood AS (
     SELECT user_id, name, f_id, f_name, order_count,
     RANK() OVER (PARTITION BY user_id ORDER BY order_count DESC) AS ranks
     FROM FoodCount
 )
 SELECT user_id, name, f_id, f_name AS favorite_food, order_count
 FROM RankedFood
 WHERE ranks = 1
...
WITH CustomerOrderCount AS (
    SELECT o.r_id, o.user_id, u.name AS customer_name, COUNT(*) AS order_count
    FROM orders o
    JOIN users u ON o.user_id = u.user_id
    GROUP BY o.r_id, o.user_id, u.name
),
RankedCustomers AS (
    SELECT r_id, user_id, customer_name, order_count,
    RANK() OVER (PARTITION BY r_id ORDER BY order_count DESC) AS ranks
    FROM CustomerOrderCount
)
SELECT r_id, customer_name, order_count
...
with Month_Revenue_Restaurant  as (
select r_id, Month(date) as Month_No, sum(amount) as Revenue 
from orders group by r_id, Month_No
),
Restaurant_Growth as (
select r_id, Month_No, Revenue, lag(Revenue) over (PARTITION BY r_id order by Month_No) as P_Month_Revenue from Month_Revenue_Restaurant
)
select r_id, Month_No, Revenue, P_Month_Revenue, (Revenue-P_Month_Revenue)/P_Month_Revenue*100 as GROWTH_BY_MONTH 
from Restaurant_Growth Where P_Month_Revenue is not Null order by r_id, Month_No;
